...
<body>
  ...
  $names = $name + " " + $surname + " " + $number_of_people;
  $addons = $table;
  if (!$fopen_dir = opendir("./csv"))
    echo "Could not open directory";
  else {
    if (!$file = fopen("./csv/names.csv", "r+"))
      echo "Error while opening the file";
    else {
      if (fwrite($file, $names) === false)
        echo "Error while writting in to file";
      else
        echo "Added result into file";
    }
    fclose($file);
    if (!$file = fopen("./csv/addons.txt", "r+"))
      echo "Error while opening the file";
    else {
      if (fwrite($file, implode($addons)) === false)
        echo "Error while writting in to file";
      else
        echo "Added result into file";
    }
